...
Public Class FrmDevTickets
    Private Sub TxtPedido_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles TxtPedido.KeyDown
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            SQL = "SELECT * FROM venta where nticket = " & CDbl(Lv_pedido)
            conn.Open()
            If conn.State = ConnectionState.Open Then
                ...
                If myDs.Tables(0).Rows.Count > 0 Then
                    If myDs.Tables(0).Rows(0)!numerofactura <> "X" Then
                        TxtPedido_C.Text = myDs.Tables(0).Rows(0)!nticket
                        DTPFecha.Value = myDs.Tables(0).Rows(0)!fecha
                        txtSubTotal_C.Text = myDs.Tables(0).Rows(0)!subtotal
                        txtIVA_C.Text = myDs.Tables(0).Rows(0)!iva
                        ...
                        Limpia_Variables_SQL_y_Cierra_Conexion()
                        SQL = "SELECT * FROM ticket where folio = " & CDbl(Lv_pedido)
                        conn.Open()
                        If conn.State = ConnectionState.Open Then
                            myCommand = New SqlCommand(SQL, conn)
                            myAdapter.SelectCommand = myCommand
                    ...
                    Else
                        ...
                        SQL = "SELECT distinct numerofactura FROM ticket where folio = " & CDbl(Lv_pedido)
                        conn.Open()
                        If conn.State = ConnectionState.Open Then
                            myCommand = New SqlCommand(SQL, conn)
                            myAdapter.SelectCommand = myCommand
                            ...
                            For i = 0 To myDs.Tables(0).Rows.Count - 1
                                If i = 0 Then
                                    lv_msg = myDs.Tables(0).Rows(i)!numerofactura
                                Else
                                    lv_msg = lv_msg & "," & myDs.Tables(0).Rows(i)!numerofactura
                                End If
                            Next i
                            MsgBox("Ticket N°:" & Trim(Lv_pedido) & ", tiene la(s) siguiente(s) factura(s) asiciada(s): (" & lv_msg & "). Favor de Generar Nota de Crédito con Referencia a Factura", MsgBoxStyle.Information, "Status de Ticket (FACTURADO).")
                            Limpia_Variables_SQL_y_Cierra_Conexion()
                        End If
                    End If
                Else
    ...
    Private Sub ImgGenerarB_Click(sender As System.Object, e As System.EventArgs) Handles ImgGenerarB.Click
        ...
        For i = 0 To DGVDetalle.Rows.Count - 1
            Sql1.Clear()
            Sql1.Append("UPDATE ticket  SET Cantidad = ")
            ...
            If Insertar_Registro(Sql1.ToString, DBConnected) = False Then
                MsgBox("No se pudo Actualizar registros en la tabla TICKET", MsgBoxStyle.Critical)
                Error_Venta = False
                Exit Sub
            End If
            Sql1.Clear()
        ...
        Sql1.Clear()
        Sql1.Append("DELETE FROM ticket ")
        Sql1.Append("where folio = ")
        ...
        If Insertar_Registro(Sql1.ToString, DBConnected) = False Then
            MsgBox("No se pudo Eliminar registros en la tabla TICKET", MsgBoxStyle.Critical)
            Error_Venta = False
            Exit Sub
        End If

        Sql1.Clear()
        Sql1.Append("UPDATE ticket SET numerofactura = NULL ")
        Sql1.Append("where folio = ")
        ...
        Sql1.Clear()
        Sql1.Append("UPDATE venta SET subtotal = ")
    ...
    Private Sub ImgSalirA_Click(sender As System.Object, e As System.EventArgs) Handles ImgSalirA.Click
