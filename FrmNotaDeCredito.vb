...
Public Class FrmNotaDeCredito
    ...
    Private Sub TxtPedido_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles TxtPedido.KeyDown
        ...
        If e.KeyCode = Keys.Enter Then
            ...
            If Not IsNothing(strFacturaTotal) Then
                ...
                TxtPedido_C.Text = CStr(strFacturaTotal.n_factura)
                DTPFecha.Value = strFacturaTotal.fecha_venta
                TxtTotal_C.Text = CStr(strFacturaTotal.total)
                ...
                lt_Facturas = DBModelo.Get_Facturas(CInt(TxtPedido.Text))
                ...
                For i = 0 To lt_Facturas.Count - 1
                    ...
                    Else
                        sObjImp = "01"
                    End If
                    Dim row As String() = New String() {lt_Facturas(i).n_factura.ToString,
                            lt_Facturas(i).idProducto.ToString,
                    ...
                    lv_iva_c = lv_iva_c + CDbl(lt_Facturas(i).IVA)
                Next i
    ...
    Private Sub DGVDetalle_CellValueChanged(sender As Object, e As System.Windows.Forms.DataGridViewCellEventArgs)
        If e.ColumnIndex = 2 And e.RowIndex <> -1 Then
            ...
            If lv_cantidad <= CDbl(DGVDetalle.Rows(e.RowIndex).Cells(6).Value) Then
                ...
                DGVDetalle.Rows(e.RowIndex).Cells(4).Value = Trim(Replace(CStr(DGVDetalle.Rows(e.RowIndex).Cells(4).Value), "$", ""))
                Dim lv_pu As Double = CDbl(DGVDetalle.Rows(e.RowIndex).Cells(4).Value)
                ...
                For i = 0 To DGVDetalle.Rows.Count - 1
                    lv_total_c = lv_total_c + CDbl(Trim(Replace(Trim(Replace(CStr(DGVDetalle.Rows(i).Cells(5).Value), "$", "")), ",", "")))
                Next i
                txtTotal.Text = Format(lv_total_c, "###,###,##0.00")
    ...
    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles mBtnGenerate.Click
        ...
        Dim NoFactura As String = CStr(FolioNC_Actual)
        ...
        Else
            ...
            For i = 0 To DGVDetalle.Rows.Count - 1
                ...
                strNC_Detalle.clave_p = CStr(DGVDetalle.Rows(i).Cells(8).Value)
                strNC_Detalle.descripcion = CStr(DGVDetalle.Rows(i).Cells(3).Value)
                ...
                strNC_Detalle.ClaveProducto = CStr(DGVDetalle.Rows(i).Cells(10).Value)
                strNC_Detalle.ClaveUnidad = CStr(DGVDetalle.Rows(i).Cells(11).Value)
        ...
        Dim wa_cliente As tblClientes = DBModelo.GetCliente(CInt(strFacturaTotal.id_cliente))
        ...
        For i = 0 To DGVDetalle.RowCount - 1
            Dim vImporte As String = Trim(Trim(CStr(DGVDetalle(5, i).Value)).Replace("$", "")).Replace(",", "").ToString
            Dim vValorUnitario As String = Trim(Trim(CStr(DGVDetalle(4, i).Value)).Replace("$", "")).Replace(",", "").ToString
            ...
            If oLinea("ObjetoImp") = "02" Then
                baseTotal = baseTotal + CDec(vImporte)
        ...
        Dim vImporteTotalIVA As String = CStr(CDbl(total) - CDbl(subtotal))
        ...
        If CDec(FormatNumber(vImporteTotalIVA, 2)) <> CDec("0.00") Then
            ...
            itra0("TasaOCuota") = FormatNumber(CDbl(FactorIVA) - 1, 6)
